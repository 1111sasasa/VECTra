...
def prepare_route_data(df,mat_padding_value,data_padding_value,max_len):
    ...
    # timestamp 对齐
    timestamp_list = []
    for ts_list in df['road_timestamp']:
        ts_tensor = torch.tensor(ts_list[:-1], dtype=torch.float32)  # 与路段对齐
        timestamp_list.append(ts_tensor)
    timestamp_data = rnn_utils.pad_sequence(timestamp_list, padding_value=0, batch_first=True)

    route_data = torch.cat([weekday_data.unsqueeze(dim=2), minute_data.unsqueeze(dim=2), delta_data.unsqueeze(dim=2),timestamp_data.unsqueeze(dim=2)], dim=-1)# (batch_size,max_len,2)
